	...
	var DataTable = function( oInit )
	{
		...
		function _fnGatherData( oSettings )
		{
			var iLoop, i, iLen, j, jLen, jInner,
			 	nTds, nTrs, nTr, nTd, aLocalData, iThisIndex,
			...
			{
			    nTr = nTrs[i];
				for ( j=0, jLen=nTr.childNodes.length ; j<jLen ; j++ )
				{
					nTd = nTr.childNodes[j];
		...
		function _fnApplyToChildren( fn, an1, an2 )
		{
			...
			{
			    nElement = an1[i];
				for ( var j=0, jLen=nElement.childNodes.length ; j<jLen ; j++ )
				{
				    nChild = nElement.childNodes[j];
